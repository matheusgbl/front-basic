...
async function renderData() {
  const data = await fetchData();
    return data.map(({ 
      ...
      const userName = metadados.users_in_photo;
      const squidUserName = usuario.username;
      const datePost = new Date(criadoEm);
      ...
      img.src = `${imagens.thumbnail.url}`;
      likes.innerHTML = `<i class="fas fa-heart"></i> ${upvotes}`;
      comments.innerHTML = `<i class="fas fa-comment"></i> ${comentarios}`;
